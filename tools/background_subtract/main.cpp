...
{
    cout
    << "Usage: ./background_subtract <input_video> <output_dir>"
    << endl;
}
...
void processFrames(vector<cv::Mat> a_frames)
{
    ...
    for (cv::Mat frame : a_frames)
    {
        ...
        if (!l_isFirstImage)
        {
            ...
            {
                cout << "File does not exist! " << l_maskFileName << endl;
...
int filenameToInt(const std::string& a_filename)
{
    string l_filename = a_filename;
    boost::replace_all(l_filename, "original_", "");
    boost::replace_all(l_filename, ".png", "");
    return boost::lexical_cast<int>(l_filename);
}

void processDirectory(
    const std::string& a_directoryName)
{
    vector<cv::Mat> l_frames;
    vector<int> l_annotations;

    fs::path p(a_directoryName);

    if(!fs::is_directory(p))
    {
        cerr << "Is not a directory, cannot annotate" << endl;
        return;
    }

    vector<string> l_filenames;
    for (auto& entry : boost::make_iterator_range(fs::directory_iterator(p), {}))
    {
        string l_strName = entry.path().filename().string();
        if (l_strName.find("mask") == string::npos &&
            l_strName.find("nobg") == string::npos &&
            l_strName.find(".png") != string::npos)
        {
            l_filenames.push_back(l_strName);
        }
    }

    // Sort numerically
    sort(l_filenames.begin(), l_filenames.end(), 
    [](const string& a_file1, const string& a_file2) -> bool
    { 
        int a = filenameToInt(a_file1);
        int b = filenameToInt(a_file2);
        return a < b; 
    });

    for (const string& filename : l_filenames)
    {
        cv::Mat l_frame = cv::imread(a_directoryName + "/" + filename);
        l_frames.push_back(l_frame);

        if (l_frames.size() % 100 == 0)
        {
            cout << "Got " << l_frames.size() << " frames" << endl;
        }
    }

    cout << "Got " << l_frames.size() << " frames" << endl;
    processFrames(l_frames);
}
...
void threadFn(char* a_videoFilename)
{
    ...
    if (!capture.isOpened())
    {
        ...
        processDirectory(a_videoFilename);
        return;
    }
